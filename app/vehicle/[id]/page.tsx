...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!vehicle) return {};
  return pageMetadata(`${vehicle.year} ${vehicle.model} ${vehicle.trim}`, `View pricing and next steps for this ${vehicle.year} ${vehicle.model}.`, `/vehicle/${id}`);
}
...
export default async function VehiclePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <section className="container">
      ...
      <p>{vehicle.price} · {vehicle.payment}</p>
      <p>{vehicle.type === 'new' ? 'New vehicle listing (feed-ready placeholder).' : `Used vehicle · ${vehicle.mileage ?? 'Mileage pending'}`}</p>
